...
1. 运行 `flask run`  
浏览器中打开页面  
![初始页面](../../images/birthdays-display.png)  
`SELECT * FROM birthdays;`查询数据库可以看到确实有三条数据。 
2. 添加数据  
![添加数据](../../images/birthdays-add.png)  
查询数据库，也有了上面新插入的数据  
```sql
...
```  
同时控制台日志也显示插入成功  
![控制台日志](../../images/birthdays-logAndSQL.png)  
3. 插入一条非法数据,返回错误提示  
...
测试，通过日志调试  
![调试日志](../../images/finace-err-log.png)  
...
![cash 不足时](../../images/cash-not-enough.png)
![buy 成功](../../images/buy.png)
## 五、首页 (index)  
**投资组合，难点在于数据聚合。**  
