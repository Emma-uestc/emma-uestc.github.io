...
### 分数（在解释中）
- `1/2` → "one half" or "one over two" or "one divided by two"
- `3/4` → "three quarters" or "three over four"
- `cents / 25` → "cents divided by twenty-five"

### 百分比
- `92 % 10` → "ninety-two mod ten" (注意：这里的%是模运算，不是百分比)
- `50%` → "fifty percent"
...
### equals (=) vs. equals (==)
- `x = 5` → "x **equals** five" or "x **gets** five" (assignment)
- `x == 5` → "x **is equal to** five" or "x **double-equals** five" (comparison)

### Divide (/) vs. Modulo (%)
- `10 / 3` → "ten **divided by** three equals three"
- `10 % 3` → "ten **mod** three equals one"

### Increment (++) vs. Add (+=)
- `i++` → "**increment** i" or "i **plus plus**"
- `i += 1` → "i **plus equals** one"

### And (&&) vs. Ampersand (&)
- `x && y` → "x **and** y" (logical AND)
- `&x` → "**address of** x" (pointer, Week 4)

### Or (||) vs. Pipe (|)
- `x || y` → "x **or** y" (logical OR)
- `x | y` → "x **bitwise or** y" (bitwise operation)
...
1. **使用完整单词**: "equals" 而不是 "="
2. **添加停顿**: 用逗号和句号控制节奏
3. **自然语言**: "while i is less than ten" 而不是 "while i < 10"
4. **明确说明**: "open parenthesis" 而不只是 "("
...
### 方法 1: 在线测试
1. 访问 Google Cloud Text-to-Speech Demo
2. 选择语言: **English (United States)**
3. 选择声音: **en-US-Neural2-D** (男声) 或 **en-US-Neural2-F** (女声)
4. 粘贴本文档中的 **TTS文本** 或 **例句**
5. 点击 "Speak" 听发音
...
**最重要的发音规则**:
1. **符号要说出来**: `<` 是 "less than"，不能跳过
2. **区分赋值和比较**: `=` vs `==`
3. **逻辑运算符要清晰**: `&&` 是 "and"，`||` 是 "or"
4. **数字要完整**: 不要说 "nine two"，要说 "ninety-two"
